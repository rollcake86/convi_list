...
class SevenApp extends StatefulWidget{
  ...
  State<StatefulWidget> createState() {
    return new SevenHome();
  }
}

class SevenHome extends State<SevenApp>{
  @override
  Widget build(BuildContext context) {
    return new Scaffold(
      appBar: new AppBar(title: new Text('SEVEN 행사'),),
      body: new Container(
        child: new FutureBuilder(
            ...
            builder: (context , snapshot){
              ...
              for(int i = 0 ; i < array.length-1 ; i++){
                if(_checkCompany(array[i], '7-ELEVEN')){
                  viewArray.add(_getProduct(array[i] , '7-ELEVEN(세븐일레븐)') +'\n' + _getCharge(array[i+1]));
                }
              }
